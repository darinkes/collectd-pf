...
print_rule(struct pf_rule *r, const char *anchor_call, int verbose, char *rulestring)
{
	...
	if (r->action > PF_NORDR)
		...
		if (r->natpass)
			snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), " pass");
	}
	if (r->action == PF_DROP) {
	...
	if (r->rt) {
		if (r->rt == PF_ROUTETO)
			...
			snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), " fastroute");
		if (r->rt != PF_FASTROUTE) {
			snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), " ");
			print_pool(&r->rpool, 0, 0, r->af, PF_PASS, rulestring);
		}
	}
	if (r->af) {
		if (r->af == AF_INET)
	...
	if (r->action == PF_SCRUB) {
		...
		if (r->rule_flag & PFRULE_FRAGDROP)
			snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), " fragment drop-ovl");
		else if (r->rule_flag & PFRULE_FRAGCROP)
			snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), " fragment crop");
		else
			snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), " fragment reassemble");
	}
	if (r->label[0])
	...
	if (!anchor_call[0] && (r->action == PF_NAT ||
		...
		print_pool(&r->rpool, r->rpool.proxy_port[0],
		    r->rpool.proxy_port[1], r->af, r->action, rulestring);
	}
}
...
print_pool(struct pf_pool *pool, u_int16_t p1, u_int16_t p2,
    sa_family_t af, int id, char *rulestring)
{
	...
	TAILQ_FOREACH(pooladdr, &pool->list, entries){
		...
		case PF_RDR:
		case PF_BINAT:
			print_addr(&pooladdr->addr, af, 0, rulestring);
			break;
		case PF_PASS:
			if (PF_AZERO(&pooladdr->addr.v.a.addr, af))
				snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), "%s", pooladdr->ifname);
			else {
				snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), "(%s ", pooladdr->ifname);
				print_addr(&pooladdr->addr, af, 0, rulestring);
				snprintf(rulestring + strlen(rulestring), 256 - strlen(rulestring), ")");

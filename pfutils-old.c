    ...
    char *rulestring)
{
	...
	if (r->action > PF_NORDR)
		...
		if (anchor_call[0] == '_') {
			...
			RULEPRINT("%s \"%s\"",
			    anchortypes[r->action], anchor_call);
	} else {
	...
	if (r->action == PF_SCRUB) {
		...
		if (r->rule_flag & PFRULE_FRAGDROP)
			RULEPRINT(" fragment drop-ovl");
		else if (r->rule_flag & PFRULE_FRAGCROP)
			RULEPRINT(" fragment crop");
		else
			RULEPRINT(" fragment reassemble");
	}
	if (r->label[0])
